...
void Shader::shaderFromFile(const char* shaderPath, GLenum shaderType, GLuint& handle){
  ...
  if(!fileToString(shaderPath, shaderSource, len)){
    throw std::string("Error loading shader: ") + shaderPath;
  }
  ...
  if(!errorFlag){
    throw "Shader compile error: " + getShaderInfoLog(handle);
  }
}
...
void Shader::linkShaders(GLuint& vertexShader, GLuint& fragmentShader, GLuint& geometryShader, GLuint& handle){
  ...
  if(!errorFlag){
    throw "Shader link error: " + getProgramInfoLog(handle);
  }
}
...
void Shader::bindUniformMatrix4(const char* name, float* m, unsigned int arraySize){
  GLuint location = glGetUniformLocation(shaderProgram, name);
  glUniformMatrix4fv(location, arraySize, GL_FALSE, m);
}

void Shader::bindUniformMatrix4(const char* name, glm::mat4 m, unsigned int arraySize){
  GLuint location = glGetUniformLocation(shaderProgram, name);
  glUniformMatrix4fv(location, arraySize, GL_FALSE, &(m[0][0]));
}
